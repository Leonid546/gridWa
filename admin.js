...
productForm.addEventListener('submit', function(event){
    ...
    xhr.open("POST", "https://market-23f4.restdb.io/rest/product");
...
xhr.open("GET", "https://market-23f4.restdb.io/rest/orders");
...
xhr.onload = function(){
    xhr.response.forEach(function(order){
            ...
            order.products.forEach(function(p){
                orderElement.innerHTML += `
                    <p><img height="50" src="${p.photo_url}">
                        ${p.name} | ${p.price}$
                    </p>
                `;
                sum += +p.price;
...
function completed(id){
    ...
    xhr.open("PUT", "https://market-23f4.restdb.io/rest/orders/"+id);
    xhr.setRequestHeader("content-type", "application/json");
    xhr.setRequestHeader("x-apikey", "659ec3e8907ee92516da4f05");
    xhr.setRequestHeader("cache-control", "no-cache");
